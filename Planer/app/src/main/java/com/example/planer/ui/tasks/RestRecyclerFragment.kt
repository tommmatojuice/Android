...
import com.example.planer.adapters.RestRecyclerAdapter
...
class RestRecyclerFragment(private var type: Int) : Fragment(), RestRecyclerAdapter.OnItemClickListener
{
    private lateinit var taskViewModel: TaskViewModel

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View?
    {
        val view = inflater.inflate(R.layout.fragment_rest_recycler, container, false)

        when(this.type){
            1 ->  taskViewModel = activity?.application?.let { TaskViewModel(it, "rest", "one_time") }!!
            2 ->  taskViewModel = activity?.application?.let { TaskViewModel(it, "rest", "fixed") }!!
            3 ->  taskViewModel = activity?.application?.let { TaskViewModel(it, "rest", "routine") }!!
        }

        val tasks = taskViewModel.taskAndGroup.value
        val adapter = this.context?.let { RestRecyclerAdapter(it, tasks, this) }
